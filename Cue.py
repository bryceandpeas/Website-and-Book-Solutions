...
compare_root = 'C:\\Users\\bryce\\Documents\\Website-Solutions'
...
date = datetime.today() - timedelta(days=259)
...
for path, subdirs, files in os.walk(compare_root):
	subdirs[:] = [d for d in subdirs if d not in exclude]
	for name in files:
		if name.endswith(".gitignore") or name.endswith(".md"):
			continue
		else:
			compare_file_list.append(os.path.join(path, name))

for file in file_list:
	if file not in compare_file_list:
...
for file in file_list:
	if file not in compare_file_list:
		...
		print('Skipping: ' + str(file) + '[Already Exists]')
		time.sleep(20)
